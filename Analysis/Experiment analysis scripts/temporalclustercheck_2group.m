            ...
            if length(sigs)>2 &&checkcluster==1
                ...
                for icl=maxClust% 1:size(clusterSTandEND,1)
                        ...
                        if shuffType==1 %null is that no condition differences.
                            ...
                            for ippant = 1:size(ttestdata,1)
                                ...
                                if mod(randi(100),2)==0 %if random even number
                                    shD(1,ippant,:) = ttestdata(ippant,:,1); % all time points.
                                    shD(2,ippant,:) = ttestdata(ippant,:,2); % all time points.
                                else
                                    shD(1,ippant,:) = ttestdata(ippant,:,2); % all time points.
                                    shD(2,ippant,:) = ttestdata(ippant,:,1); % all time points.
                                end
                    ...
                    if observedCV>H.Data(cv05uncorr)
                        disp(['sum tvals = ' num2str(observedCV)]);

                        yl=get(gca, 'ylim');
                        xl=get(gca, 'xlim');
                        sigplace = yl(1) + .1*(diff(yl)); % place at bottom for catch
                        
                       %%
                       for itime=checktimes

                           hold on
                           plot(use_xvec(itime), sigplace, ['*' ],'markersize', 10, 'linewidth', 3, 'color', 'k')
